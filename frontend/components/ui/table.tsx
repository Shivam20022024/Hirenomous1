...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
  return (
    <thead
      ...
      className={cn('bg-muted/60 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground', className)}
      {...props}
    />
  )
}
...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  return <th data-slot="table-head" className={cn('px-4 py-3 font-semibold', className)} {...props} />
}
